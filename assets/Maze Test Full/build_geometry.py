...
def build():
    ...
    try:
        ...
        polys = (
            list(walkable.geoms) if walkable.geom_type == "MultiPolygon" else [walkable]
        )
        for poly in polys:
            xs, ys = poly.exterior.xy
            ax.add_patch(
                MplPoly(
                    list(zip(xs, ys)), facecolor="#c8d5b9", edgecolor="#2d4a1e", lw=1.2
                )
            )
        ...
        ax.set_title(
            "Corn Maze -- 50 x 50 m  (green = walkable path, dark = corn/wall)",
            fontsize=10,
        )
