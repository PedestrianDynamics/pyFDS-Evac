...
def draw_plan(ax, verts, s, x, y, v0=ROUTING.base_speed_m_per_s):
    ...
    ax.add_patch(Rectangle((0, 0), 11, 5.6, fc="white", ec=BLUE, lw=1.6, zorder=0))
    # internal wall separating the entrance hall from the corridor
    ax.add_patch(Rectangle((3.0, 2.4), 6.0, 0.25, fc="#3b3b3b", ec="none", zorder=3))
    ax.add_patch(Rectangle((3.0, 2.4), 0.25, 3.2, fc="#3b3b3b", ec="none", zorder=3))
    ...
    ax.plot(verts[:, 0], verts[:, 1], color=BLUE, lw=2.0, zorder=4)
    sc = ax.scatter(
        x,
        y,
        c=k,
    ...
    # stage-graph nodes
    ax.scatter([2.6, 9.6], [3.0, 1.4], s=70, fc="white", ec=BLUE, lw=1.6, zorder=6)
    ax.text(
        2.35, 2.75, "node $u$", fontsize=8, color=BLUE, ha="right", va="top", zorder=6
    )
    ax.text(
        9.6, 1.05, "node $v$", fontsize=8, color=BLUE, ha="center", va="top", zorder=6
    )
    ax.add_patch(Rectangle((9.2, 5.5), 0.8, 0.2, fc=GREEN, ec="none", zorder=6))
    ax.text(9.05, 5.35, "exit", fontsize=8, color=GREEN, ha="right", va="top", zorder=6)

    # agent and its walkable distance to the first node
    ax.scatter(
        [verts[0, 0]], [verts[0, 1]], s=90, fc=ORANGE, ec="white", lw=1.2, zorder=7
    )
    ax.text(
        verts[0, 0] - 0.15,
        verts[0, 1] + 0.3,
    ...
    ax.annotate(
        ...
        xytext=(1.05, 4.05),
        arrowprops=dict(
            arrowstyle="<->", color=GREY, lw=1.0, connectionstyle="arc3,rad=0.35"
        ),
        zorder=6,
    )
    ax.text(
        1.35, 3.1, "$d_W$", fontsize=9, color=GREY, ha="center", va="center", zorder=6
    )
...
def draw_spacetime(ax, s, x, y, v0=ROUTING.base_speed_m_per_s, period=1.0, t0=6.0):
    ...
    ax.plot([0, s_max], [t0, t0 + s_max / v0], color=BLUE, lw=2.0, zorder=4)
    ax.scatter(
        s,
        t0 + s / v0,
    ...
    ax.plot([0, s_max], [t0, t0], color=ORANGE, lw=1.4, ls="--", zorder=4)
    ax.text(
        s_max * 0.25,
        t0 - 1.0,
        "frozen field at $t$",
    ...
    for k in range(0, 8):
        ...
        ax.plot([0.1, 0.45], [tk, tk], color=ORANGE, lw=1.6, zorder=6)
    ax.text(
        0.7,
        t0 + 7 * 3 * period * 0.62,
        "re-decide\nevery $T$",
    ...
    ax.set_xlim(0, s_max)
    ax.set_ylim(0, 40)
    ax.set_xlabel("arc length $s$ along the route [m]", fontsize=8.5, color=GREY)
    ax.set_ylabel("time [s]", fontsize=8.5, color=GREY)
...
def main():
    ...
    cbar.set_label(
        r"extinction $K_p$ at the sample [m$^{-1}$]", fontsize=8.5, color=GREY
    )
