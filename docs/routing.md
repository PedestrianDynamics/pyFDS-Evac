...
where nodes are stages (distributions, checkpoints, exits) and edge
weights are Euclidean distances between stage centroids. The graph is
built once at simulation start from the scenario configuration.
...
The graph provides Dijkstra-based shortest-path queries:
...
For each segment (edge between two stages), the system:

1. Samples the extinction coefficient K along the line of sight
   between stage centroids using the Beer-Lambert path-integrated
   mean (Boerger et al. 2024, Eq. 8-9).
2. Computes the smoke-adjusted speed factor from the mean K.
3. Estimates the travel time from the segment length and reduced
   speed.
4. Optionally estimates the FED growth along the segment from the
...
1. Determines the agent's current position in the stage graph.
2. Ranks all routes from that position using `rank_routes`.
3. If the best route leads to a different exit, reroutes the agent
   by updating its path choices and target stage.
4. Returns a `RouteSwitch` record for diagnostics, or `None` if no
   switch occurred.
...
| `path`             | `list[str]`      | Ordered stage IDs from source to exit |
